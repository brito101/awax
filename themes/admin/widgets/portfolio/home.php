...
<section class="dash_content_app">
    <header class="dash_content_app_header">
        <h2 class="icon-tag">Portfólio</h2>
        <form action="<?= url("/admin/portfolio/home"); ?>" method="post" class="app_search_form">
            <input type="text" name="s" value="<?= $search; ?>" placeholder="Pesquisar Portfólio:">
    ...
    <div class="dash_content_app_box">
        <section>
            <div class="app_blog_home">
                <?php if (!$posts): ?>
                    <div class="message info icon-info">Ainda não existem artigos cadastrados no portfólio.</div>
                <?php else: ?>
                    <?php foreach ($posts as $post):
                        ...
                        <article>
                            ...
                            <h3 class="tittle">
                                <a target="_blank" href=" <?= url("/portfolio/{$post->uri}"); ?>">
                            ...
                            <div class="actions">
                                <a class="icon-pencil btn btn-blue" title=""
                                   href="<?= url("/admin/portfolio/post/{$post->id}"); ?>">Editar</a>

                                <a class="icon-trash-o btn btn-red" title="" href="#"
                                   data-post="<?= url("/admin/portfolio/post"); ?>"
                                   data-action="delete"
