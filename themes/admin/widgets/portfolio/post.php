<?php $v->layout("_admin"); ?>
<?php $v->insert("widgets/portfolio/sidebar.php"); ?>

<div class="mce_upload" style="z-index: 997">
    <div class="mce_upload_box">
        <form class="app_form" action="<?= url("/admin/portfolio/post"); ?>" method="post" enctype="multipart/form-data">
...
<section class="dash_content_app">
    <?php if (!$post) : ?>
        <header class="dash_content_app_header">
            <h2 class="icon-plus-circle">Novo Projeto</h2>
        </header>

        <div class="dash_content_app_box">
            <form class="app_form" action="<?= url("/admin/portfolio/post"); ?>" method="post">
                <!-- ACTION SPOOFING-->
    ...
    <?php else : ?>
        <header class="dash_content_app_header">
            ...
            <a class="icon-link btn btn-green" href="<?= url("/portfolio/{$post->uri}"); ?>" target="_blank" title="">Ver no
                site</a>
        </header>

        <div class="dash_content_app_box">
            <form class="app_form" action="<?= url("/admin/portfolio/post/{$post->id}"); ?>" method="post">
                <!-- ACTION SPOOFING-->
                <input type="hidden" name="action" value="update" />

                <label class="label">
                    <span class="legend">*Título:</span>
                    <input type="text" name="title" value="<?= $post->title; ?>" placeholder="A manchete do seu artigo" required />
                </label>

                <label class="label">
                    <span class="legend">*Subtítulo:</span>
                    <textarea name="subtitle" placeholder="O texto de apoio da manchete" required><?= $post->subtitle; ?></textarea>
                </label>
                ...
                <div class="label_g2">
                    ...
                    <label class="label">
                        <span class="legend">Data de publicação:</span>
                        <input class="mask-datetime" type="text" name="post_at" value="<?= date_fmt($post->post_at, "d/m/Y H:i"); ?>" required />
                    </label>
                </div>
...
<script src="<?= theme("/assets/minify/tinymce.js", CONF_VIEW_ADMIN); ?>"></script>
<?php $v->end(); ?>